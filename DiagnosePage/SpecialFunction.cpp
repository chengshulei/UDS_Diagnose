...
SpecialFunction::SpecialFunction(QWidget *parent)
{
	ui.setupUi(this);
	...
	connect(this, SIGNAL(specialfunction_result_Signal(bool, QString)), this, SLOT(specialfunction_result_Slot(bool, QString)));
...
void SpecialFunction::Init()
{
	specialfunction_command_vector = deal_settingfile.AnalyseDiagnoseSettingFile("SpecialFunction");
	QVBoxLayout *mainLayout = new QVBoxLayout;
	for (int i = 0; i < specialfunction_command_vector.size() / 5 + 1; i++)
	{
		QHBoxLayout* temp_hlayout = new QHBoxLayout;
		for (int j = 0; j < 5 && i * 5 + j < specialfunction_command_vector.size(); j++)
		{
			...
			temp_button->setText(specialfunction_command_vector[i].Caption_content);
			temp_hlayout->addItem(temp_horizontalSpacer);
			temp_hlayout->addWidget(temp_button);
			connect(temp_button, SIGNAL(clicked()), this, SLOT(specialfunctionButtontSlot()));
...
void SpecialFunction::startThread()//启动线程
{
	boost::function0<void> f1 = boost::bind(&SpecialFunction::deal_send_receive, this);
	specialfunction_thread = boost::thread(f1);
}

void SpecialFunction::TableWrite(QString message_content, int message_type)//弹出警告信息窗口
{
	emit display_messageSignal(message_content, message_type);
}

void SpecialFunction::specialfunctionButtontSlot()//特殊功能按钮
{
	QObject* temp_obj = sender();
	QString function_name = ((QPushButton*)temp_obj)->text();
	for (int i = 0; i < specialfunction_command_vector.size(); i++)
	{
		if (specialfunction_command_vector[i].Caption_content == function_name)
		{
			LoaderData temp_command = specialfunction_command_vector[i];
...
void SpecialFunction::specialfunction_result_Slot(bool flag, QString function_name)//特殊功能操作结果
{
	if (!flag)
	{
...
void SpecialFunction::deal_send_receive()
{
	int re_send_item = 0;
		...
		if (temp_receive_settingdata.frame_id != 0)//接收数据帧不为，表示接收不为空，需要获得相应数据帧并解析
		{
			...
			else
			{
				...
				if (temp_receive_byte_vector[0] != 0x10 && current_command.send_data.hex_data[0] != 0x30 && temp_receive_byte_vector.size() == 8)//普通单帧接收
				{
					...
					{
						specialfunction_result_Signal(true, this_command.Caption_content);
						receive_byte_vector.clear();
						break;
					}
					else
					{
						specialfunction_result_Signal(false, this_command.Caption_content);
						receive_byte_vector.clear();
						break;
					}
				}
				else if (current_command.send_data.hex_data[0] == 0x30)//多帧中，0x30发送帧之后的接收数据帧
				{
					...
					if (DealReceiveData(temp_receive_settingdata, receive_byte_vector))
					{
						emit specialfunction_result_Signal(true, this_command.Caption_content);
						receive_byte_vector.clear();
						break;
					}
					else
					{
						emit specialfunction_result_Signal(false, this_command.Caption_content);
						receive_byte_vector.clear();
						break;
					}
				}
...
bool SpecialFunction::DealReceiveData(ReceiveDiagloseData temp_receive_data, QVector<BYTE> receive_byte_vector)//发送过程
{
...
void SpecialFunction::send_realtime_data(SendData send_data)//发送过程
{
...
QVector<BYTE> SpecialFunction::receive_realtime_data(SendData send_data, ReceiveDiagloseData temp_receive_data)//接收过程
{
	QVector<BYTE> temp_byte_vector;
	int find_send_frame_index = -1;
	for (int m = 0; m < specialfunction_Record_Vector.size(); m++)//在获取的发送接收数据帧中，进行筛选
	{
		Record temp_can_record = specialfunction_Record_Vector[m];
		int same_byte_count = 0;
	...
	if (find_send_frame_index >= 0)//找到发送帧
	{
		...
		for (int m = find_send_frame_index; m < specialfunction_Record_Vector.size(); m++)//从发送帧位置后开始查找接收帧
		{
			if (specialfunction_Record_Vector[m].this_frame.ID == current_page_block.receive_frame_id)//在接收数据帧中查找指定ID
			{
				if (specialfunction_Record_Vector[m].this_frame.Data[0] == 0x10)//多帧的首帧
				{
					for (int j = 0; j < 8; j++)
					{
						temp_byte_vector.push_back(specialfunction_Record_Vector[m].this_frame.Data[j]);
					}
					muti_frame_byte_number = specialfunction_Record_Vector[m].this_frame.Data[1];//多帧总字节数
					muti_frame_byte_counter = 6;//当前帧中包含6个多帧有效字节
					specialfunction_Record_Vector.remove(0, m + 1);//清空发送接收帧容器
					break;
				}
				else if (send_data.hex_data[0] == 0x30 &&
					(specialfunction_Record_Vector[m].this_frame.Data[0] >= 0x20
					&& specialfunction_Record_Vector[m].this_frame.Data[0] <= 0x2f))//多帧的，0x30发送帧对应的接收帧
				{
					for (int j = 1; j < 8; j++)
					{
						temp_byte_vector.push_back(specialfunction_Record_Vector[m].this_frame.Data[j]);
...
void SpecialFunction::Get_Record()//获取发送接收数据帧，这里进行帧ID过滤
{
	record_mutex.lock();
	for (int i = 0; i < CAN_Record.size(); i++)
	{
		...
		if (current_page_block.send_frame_id != 0 && temp_record.this_frame.ID == current_page_block.send_frame_id && temp_record.frame_index > max_index)//这里根据帧ID和索引，避免重复获取
		{
			...
			specialfunction_Record_Vector.push_back(temp_record);
		}
