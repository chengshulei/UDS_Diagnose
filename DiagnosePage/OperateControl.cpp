...
void OperateControl::TableWrite(QString message_content, int message_type)//弹出警告信息窗口
{
	emit display_messageSignal(message_content, message_type);
}

void OperateControl::communicationcontrolButtontSlot()//特殊功能按钮
{
...
void OperateControl::communicationcontrol_result_Slot(bool flag, QString function_name)//特殊功能操作结果
{
	if (!flag)
	{
...
bool OperateControl::DealReceiveData(ReceiveDiagloseData temp_receive_data, QVector<BYTE> receive_byte_vector)//发送过程
{
...
void OperateControl::send_realtime_data(SendData send_data)//发送过程
{
...
QVector<BYTE> OperateControl::receive_realtime_data(SendData send_data, ReceiveDiagloseData temp_receive_data)//接收过程
{
...
void OperateControl::Get_Record()//获取发送接收数据帧，这里进行帧ID过滤
{
	record_mutex.lock();
...
QVector<BYTE> OperateControl::analyse_send_singleframe(QString data_content)//解析单帧
{
